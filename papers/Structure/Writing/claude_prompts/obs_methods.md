...
### 2026-06-26 (Item 9: reproducibility pass on the Observations & Methods text)

Re-read §3 (Data + Structure functions) for reproducibility and made targeted additions to §3b (compiles clean, 14 pages):
1. **Velocity / separation defined:** added that $\uvec=(u,v)$ is the horizontal ADCP velocity, $r\equiv|\rvec|$ the horizontal distance between profile positions, $\rhat\equiv\rvec/r$, with a pointer to §3a for the gridded-velocity processing.
2. **Pair bookkeeping:** stated each unordered pair enters once (only positive time differences — no double counting), and pairs are accumulated over all gliders/times subject to the distinct-glider and $\Delta t$ criteria.
3. **Bin spec made precise + removed an over-claim:** changed "linear bins … widening the outermost bin where pairs become sparse" to "\valnbins\ uniform (linearly spaced) bins spanning 0–\valrmax\ ($\approx5$~km wide)". The widening was **never implemented** in the code/figures, so the claim was inaccurate.

**⚠️ KEY REPRODUCIBILITY ISSUE FLAGGED (needs JXP decision, not silently changed).** The Methods text states **linear** binning is adopted (per Q3), but every result figure is generated with **`struct_defs.btype='log'`** — i.e. 40 log-spaced bins (1–400 km), not the linear scheme the text describes. So `fig_duL`, `fig_duL2` (incl. the $p\approx0.42$ fit), `fig_duL3`, `fig_corrected_s3`, `fig_S1S3_other`, and `fig_separations` are all **log-binned**, contradicting the text and the reported "19 good bins". To reproduce the *shown* results one currently needs `btype='log'`. Resolution requires a choice: (a) set `struct_defs.btype='lin'` and **regenerate all result figures** (changes every panel, the good-bin set, and the fit slope) to match the adopted-linear text; or (b) change the text/Q3 decision to state log binning as primary. I did **not** change the scheme or regenerate, since it alters all results and the displayed numbers — recommend (a) to honor the stated linear choice, pending confirmation.

**Other notes (not blocking):** (i) the bootstrap-resampled-mean central value (logged 06-25) gives small run-to-run jitter — a code detail, not stated in the text; (ii) the survey-edge cuts (Calypso 2022 drops first 3 / last 12 days) and good-velocity cut are reproducibility-relevant but belong in the **Data** subsection (still a deferred stub) — the Table notes mention them generically.

**Not pushed** (item 9 has no git instruction). Working tree now holds: `energy_cascade.tex` (Structure-functions reproducibility edits + the `\input{tab_experiments}` + the `\dtysm` typo fix), the new `tab_experiments.tex`, and the five regenerated `minN=100` result figures. Overleaf remains broken until pushed.